...
const (
	DEFAULT_GEN_CONTROLLER_PATH = "./app/controller"
)

// doGenModel implements the "gen model" command.
func doGenController(parser *gcmd.Parser) {
	var err error
	genPath := parser.GetArg(3, DEFAULT_GEN_CONTROLLER_PATH)
	...
	for _, table := range tables {
		...
		generateControllerContentFile(db, table, variable, genPath, configGroup)
	}
	mlog.Print("done!")
}
...
func generateControllerContentFile(db gdb.DB, table, variable, folderPath, groupName string) {
...
import (
	...
	"suyuan/app/utils/response"
`)

		modelPackageImports += "\n" +
			`	"suyuan/app/model/`+packageName+`"` +
			"\n)"

		glog.Debugf("-----TplPackageImports",modelPackageImports)

		//空白model
		indexContent := gstr.ReplaceByMap(templateControllerContent, g.MapStrStr{
			"{TplTableName}":      table,
